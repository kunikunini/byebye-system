...
export default function ItemEditForm({ item }: { item: Item }) {
    ...
    const handleDiscogsSearch = async () => {
        ...
        try {
            ...
            if (data.results && data.results.length > 0) {
                ...
                alert('Discogs APIトークンが設定されていません');
            } else {
    ...
    // Helper for formatting prices based on Discogs rules (supports both API objects and scraped strings)
    const formatDiscogsPrice = (rawPrice: any) => {
        if (!rawPrice) return { display: '-', sub: '' };

        // 1. Check if it's already a formatted string from scraping (e.g. "¥1,234" or "$12.34")
        if (typeof rawPrice === 'string') {
            ...
            if (str.includes('$') || str.includes('USD')) {
                ...
                if (!isNaN(numeric)) {
                    return {
                        display: `¥${Math.round(numeric * 150).toLocaleString()}`,
                        sub: str
                    };
                }
                return { display: str, sub: '' };
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="rounded-2xl border border-gray-100 bg-white p-6 shadow-sm">
                    <div className="flex items-center justify-between mb-6">
                        <div className="flex items-center gap-3">
                            <div className="flex h-8 w-8 items-center justify-center rounded-xl bg-gold-2 text-[14px] font-black text-black">¥</div>
                            <h3 className="text-lg font-black text-gray-900">マーケットプレイス相場</h3>
                        </div>
                        <button
                            type="button"
                            onClick={handleFetchPriceSuggestions}
                            disabled={!selectedReleaseId || isFetchingPrice}
                            className={`flex items-center gap-2 rounded-xl px-5 py-2.5 text-sm font-black transition-all active:scale-95 disabled:opacity-50 ${selectedReleaseId ? 'bg-black text-white hover:bg-gray-800' : 'bg-gray-50 text-gray-400'}`}
                        >
                    ...
                    {priceSuggestions ? (
                        <div className="space-y-6">
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                <div className="rounded-2xl border border-pink-100 bg-pink-50/20 p-3 text-center">
                                    <div className="text-[10px] font-black text-pink-400 uppercase mb-1">Want list</div>
                                    <div className="text-lg font-black text-pink-600">{priceSuggestions.stats?.num_want ?? '-'}</div>
                                </div>
                                <div className="rounded-2xl border border-blue-100 bg-blue-50/20 p-3 text-center">
                                    <div className="text-[10px] font-black text-blue-400 uppercase mb-1">Collection</div>
                                    <div className="text-lg font-black text-blue-600">{priceSuggestions.stats?.num_have ?? '-'}</div>
                                </div>
                            </div>
                            ...
                            <div className="rounded-2xl bg-gray-50/50 p-5 border border-gray-100">
                                <div className="flex items-center justify-between mb-4">
                                    <h4 className="text-xs font-black text-gray-500 uppercase tracking-widest">現在の出品状況</h4>
                                    {priceSuggestions.stats?.num_for_sale !== undefined && (
                                        <span className="text-[10px] font-bold text-green-600">{priceSuggestions.stats.num_for_sale} 点が販売中</span>
                                    )}
                                </div>
                                <div className="flex items-baseline gap-2">
                                    <span className="text-[10px] font-bold text-gray-400 uppercase">販売中の最安値:</span>
                                    {(() => {
                            ...
                            <div className="rounded-2xl bg-white p-5 border border-gray-100 shadow-sm relative">
                                <div className="flex items-center justify-between mb-5">
                                    <h4 className="text-xs font-black text-gray-500 uppercase tracking-widest">過去の売買統計 (Sales History)</h4>
                                    <div className="text-[10px] font-bold text-gray-400">最終販売: <span className="text-gray-900 font-black">{priceSuggestions.stats?.last_sold ?? 'なし'}</span></div>
                                </div>
                                    ...
                                    {[
                                        ...
                                        return (
                                            <div key={stat.key} className="space-y-1">
                                                <div className="text-[10px] font-black text-gray-400 uppercase tracking-tighter">{stat.label}</div>
                                ...
                                {priceSuggestions.scraped === false && (
                                    <p className="mt-4 text-[9px] text-gray-400 font-medium italic">
                                        ※ API制限により「中間点・最高値」の取得に失敗しました。Discogsサイトを直接ご確認ください。
                                    </p>
                                )}
                            </div>

                            <div className="flex items-center justify-between px-1">
                                <p className="text-[9px] text-gray-400 font-medium">※ ドル表示（$）のデータは 1ドル=150円 で参考換算しています。</p>
                                <details className="group">
                                    <summary className="cursor-pointer text-gray-300 text-[9px] font-black uppercase list-none hover:text-gray-500">Show API Debug</summary>
                                    <pre className="absolute left-6 right-6 bottom-full mb-4 max-h-60 overflow-auto rounded-2xl bg-black/95 p-5 text-[9px] font-mono text-green-400 shadow-2xl backdrop-blur-xl border border-white/10">
                        ...
                        <div className="flex flex-col items-center justify-center py-12 text-center bg-gray-50/50 rounded-2xl border border-dashed border-gray-200">
                            <p className="max-w-[240px] text-xs font-bold text-gray-400 leading-relaxed">
                                {selectedReleaseId ? "「相場を更新」ボタンを押すと、サイト上の販売統計情報を取得します。" : "まず上部の品番・Discogs検索で商品を特定してください。"}
                            </p>
                        </div>
                    )}
                </div>
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                    <div>
                        ...
                        <input name="storage_location" defaultValue={item.storageLocation ?? ''} className="mt-1 w-full rounded border border-gray-200 bg-white px-3 py-2 focus:ring-1 focus:ring-gold-2 focus:outline-none" />
                    </div>
                    <div className="sm:col-span-2">
                        <label className="block text-sm font-medium text-text-secondary">notes</label>
                        <textarea name="notes" defaultValue={item.notes ?? ''} className="mt-1 w-full rounded border border-gray-200 bg-white px-3 py-2 focus:ring-1 focus:ring-gold-2 focus:outline-none" rows={3} />
                    </div>
                    <div>
                        <label className="block text-sm font-medium text-text-secondary">status</label>
                        <select name="status" defaultValue={item.status} className="mt-1 rounded border border-gray-200 bg-white px-3 py-2 focus:ring-1 focus:ring-gold-2 focus:outline-none">
                ...
                <div className="flex items-center justify-start gap-4 border-t border-gray-100 pt-6">
                    ...
                    <button className="rounded-xl bg-black px-8 py-3 text-sm font-bold text-white shadow-lg hover:scale-105 active:scale-95 transition-all" type="submit" disabled={isLoading}>
                        {isLoading ? "保存中..." : "保存する"}
                    </button>
